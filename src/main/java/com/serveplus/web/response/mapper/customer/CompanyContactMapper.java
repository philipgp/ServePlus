...
public class CompanyContactMapper implements Mapper<ContactVO,CompanyContact>{

	private Company company;
	public CompanyContactMapper(Company company) {
		super();
		this.company = company;
	}
	@Override
	public CompanyContact mapFrom(ContactVO source) {
		ContactMapper contactMapper = new ContactMapper();
		ContactDetail contactDetails = contactMapper.mapFrom(source);
		CompanyContact userContact = new CompanyContact();
		...
		userContact.setCompany(company);
		return userContact;
	}
